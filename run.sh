...
if [ -n "${WERCKER_AWS_CLOUDFORMATION_TEMPLATE_BODY:+1}" ]; then
  WERCKER_AWS_CLOUDFORMATION_TEMPLATE_ARG="--template-body $WERCKER_AWS_CLOUDFORMATION_TEMPLATE_BODY"
else
  if [ -n "${WERCKER_AWS_CLOUDFORMATION_TEMPLATE_URL:+1}" ]; then
    WERCKER_AWS_CLOUDFORMATION_TEMPLATE_ARG="--template-url $WERCKER_AWS_CLOUDFORMATION_TEMPLATE_URL"
  else
